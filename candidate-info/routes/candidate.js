...
var db = mongojs('mongodb://localhost:27017/candidateInformationTable', [
  'candidateInformationTables', 'evaluationSheetInformationTables',
  'managerEvaluationInformationTables', 'hrEvaluationInformationTables',
  'users', 'candidateInterviewRounds'
]);
...
router.get('/candidateInfo/newManagerForm', function(req, res, next) {
  ...
  db.managerEvaluationInformationTables.find(function(err,
    managerEvaluationInformationTables) {
    if (err) {
      res.send(err);
    }
...
router.get('/candidateInfo/newHREvaluationForm', function(req, res, next) {
  ...
  db.hrEvaluationInformationTables.find(function(err,
    hrEvaluationInformationTables) {
    if (err) {
      res.send(err);
    }
...
router.post('/candidateInfo/newHREvaluationForm', function(req, res, next) {
  ...
  db.hrEvaluationInformationTables.save(hrEvaluator, function(err,
    hrEvaluator) {
    if (err) {
      res.send(err);
    }
...
router.post('/candidateInfo/CandidateRounds', function(req, res, next) {
  ...
  db.candidateInterviewRounds.save(interviewRounds, function(err,
    interviewRounds) {
    if (err) {
      res.send(err);
    }
...
router.post('/candidateInfo/newManagerForm', function(req, res, next) {
  ...
  db.managerEvaluationInformationTables.save(managerEvaluator, function(err,
    managerEvaluator) {
    if (err) {
      res.send(err);
    }
...
router.get('/candidateInfo', passport.authenticate('jwt', {
  session: false
}), function(req, res) {
  var token = getToken(req.headers);
  if (token) {
    db.candidateInformationTables.find(function(err,
      candidateInformationTables) {
      if (err) {
        res.send(err);
      }
...
router.get('/candidateInfo/newIAForm', function(req, res, next) {
  db.evaluationSheetInformationTables.find(function(err,
    evaluationSheetInformationTables) {
    if (err) {
      res.send(err);
    }
...
router.get('/candidateInfo/newIAForm/:id', function(req, res, next) {
  ...
  db.evaluationSheetInformationTables.findOne({
    _id: mongojs.ObjectId(req.params.id)
  }, function(err, evaluator) {
    if (err) {
      res.send(err);
    }
    res.json(evaluator);
...
// Get Single Task
router.get('/candidateInfo/:id', passport.authenticate('jwt', {
  session: false
}), function(req, res) {
  var token = getToken(req.headers);
  if (token) {
    db.candidateInformationTables.findOne({
      _id: mongojs.ObjectId(req.params.id)
...
router.delete('/candidateInfo/newIAForm/:id', function(req, res, next) {
  db.evaluationSheetInformationTables.remove({
    _id: mongojs.ObjectId(req.params.id)
  }, function(err, IAdata) {
    if (err) {
      res.send(err);
...
router.delete('/candidateInfo/newManagerForm/:id', function(req, res, next) {
  db.managerEvaluationInformationTables.remove({
    _id: mongojs.ObjectId(req.params.id)
  }, function(err, managerEvaluationData) {
    if (err) {
      res.send(err);
    }
...
router.delete('/candidateInfo/newHREvaluationForm/:id', function(req, res, next) {
  db.hrEvaluationInformationTables.remove({
    _id: mongojs.ObjectId(req.params.id)
  }, function(err, hrEvaluationData) {
    if (err) {
      res.send(err);
    }
...
router.delete('/candidateInfo/CandidateRounds/:id', function(req, res, next) {
  db.candidateInterviewRounds.remove({
    _id: mongojs.ObjectId(req.params.id)
  });
});
...
router.delete('/candidateInfo/:id', function(req, res, next) {
  db.candidateInformationTables.remove({
    _id: mongojs.ObjectId(req.params.id)
  }, function(err, candidate) {
    if (err) {
      res.send(err);
...
router.put('/candidateInfo/:id', function(req, res, next) {
  ...
  if (!updcandidateInfo) {
    ...
    db.candidateInformationTables.update({
      _id: mongojs.ObjectId(req.params.id)
    }, updcandidateInfo, {}, function(err, candidate) {
      if (err) {
        res.send(err);
      }
      res.json(candidate);
...
router.post('/candidateInfo/newIAForm', function(req, res, next) {
  ...
  db.evaluationSheetInformationTables.save(evaluator, function(err,
    evaluator) {
    if (err) {
      res.send(err);
    }
  ...
  console.log('inside server evaluator save======================',
    evaluator)
});
...
router.put('/candidateInfo/newIAForm/:id', function(req, res, next) {
  var evaluator = req.body;
  let updatedIA = {};
  console.log('inside server evaluator update--------------------',
    evaluator)
  ...
  if (evaluator.createdBy) {
    updatedIA.createdBy = evaluator.createdBy;
  }
  ...
  if (!Object.keys(updatedIA).length) {
    ...
    db.evaluationSheetInformationTables.update({
      _id: mongojs.ObjectId(req.params.id)
    }, updatedIA, function(err, evaluator) {
      if (err) {
        res.send(err);
      }
      res.json(evaluator);
  ...
  //     res.json(evaluator);
  // });
  console.log('inside update----------------------------evaluator',
    evaluator)
});
...
router.put('/candidateInfo/newManagerForm/:id', function(req, res, next) {
  ...
  if (!Object.keys(updatedManagerInfo).length) {
    ...
    db.managerEvaluationInformationTables.update({
      _id: mongojs.ObjectId(req.params.id)
    }, updatedManagerInfo, function(err, managerEvaluator) {
      if (err) {
        res.send(err);
      }
      res.json(managerEvaluator);
...
router.put('/candidateInfo/newHREvaluationForm/:id', function(req, res, next) {
  ...
  if (hrEvaluator.createdBy) {
    updatedHRInfo.createdBy = hrEvaluator.createdBy;
  }
  ...
  if (!Object.keys(updatedHRInfo).length) {
    ...
    db.hrEvaluationInformationTables.update({
      _id: mongojs.ObjectId(req.params.id)
    }, updatedHRInfo, function(err, hrEvaluator) {
      if (err) {
        res.send(err);
      }
      res.json(hrEvaluator);
...
router.put('/candidateInfo/CandidateRounds/:id', function(req, res, next) {
  ...
  // var updcandidateInfo = {};
  console.log('in update method of round===================',
    candidateRounds);

  db.candidateInterviewRounds.update({
    _id: mongojs.ObjectId(candidateRounds._id)
  }, candidateRoundsIAdata, function(err, candidateRounds) {
    if (err) {
      res.send(err);
    }
